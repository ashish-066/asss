...
class PopupManager {
  ...
  async saveData() {
    try {
      ...
      // Also notify background script to reload data
      try {
        await chrome.runtime.sendMessage({ type: 'reloadData' });
        console.log('📡 Notified background script to reload data');
      } catch (error) {
  ...
  updateVolunteersList() {
    ...
    volunteersList.innerHTML = this.volunteers.map(volunteer => `
      <div class="volunteer-item">
        ...
        <button class="btn btn-danger btn-small" onclick="popupManager.removeVolunteer('${volunteer.id}')" style="margin-top: 10px;">
          Remove
        </button>
      </div>
    `).join('');
