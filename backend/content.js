...
console.log("📤 Sending URLs to background script:", links.length, "URLs");
chrome.runtime.sendMessage({ type: "urlsCaptured", urls: links }, (response) => {
  ...
  document.getElementById('request-otp-btn').addEventListener('click', async () => {
    ...
    try {
      ...
      console.log('📤 Message payload:', { type: 'requestOTP', url: url, reason: reason });
          ...
          // Skip storage check for now - focus on OTP
          console.log('⏭️ Skipping storage check - proceeding with OTP request');
